...
impl BlockchainClient for PolygonPosClient {
    ...
    fn create_config(network: &str) -> Result<BlockchainConfig> {
        rosetta_config_polygon_pos::config(network)
    }
...
mod tests {
    ...
    async fn test_network_list() -> Result<()> {
        let config = rosetta_config_polygon_pos::config("dev")?;
    ...
    async fn test_network_options() -> Result<()> {
        let config = rosetta_config_polygon_pos::config("dev")?;
        rosetta_server::tests::network_options::<PolygonPosClient>(config).await
    }

    #[tokio::test]
    async fn test_network_status() -> Result<()> {
        let config = rosetta_config_polygon_pos::config("dev")?;
        rosetta_server::tests::network_status::<PolygonPosClient>(config).await
    }
