...
impl Precompile for Bn128Add {
    fn execute(handle: &mut impl PrecompileHandle) -> PrecompileResult {
        ...
        handle.record_cost(Self::GAS_COST)?;
...
impl Precompile for Bn128Pairing {
    fn execute(handle: &mut impl PrecompileHandle) -> PrecompileResult {
        ...
        let ret_val = if handle.input().is_empty() {
            handle.record_cost(Self::BASE_GAS_COST)?;
            U256::one()
        } else {
            ...
            let gas_cost: u64 =
                Self::BASE_GAS_COST + (elements as u64 * Self::GAS_COST_PER_PAIRING);
...
mod tests {
    use super::{
        super::test_utils::test_precompile_test_vectors, Bn128Add, Bn128Mul, Bn128Pairing,
    };
