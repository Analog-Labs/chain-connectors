...
/// * `schnorr_1`: `r (32-bytes) || s (32-bytes)` - `64 bytes`  (schnorr signature implemented by
///   Zilliqa where both `r` and `s` are scalars encoded as `32-bytes` values, most significant byte
///   first.)
...
///   `s` are scalars encoded as `32-bytes` values, least significant byte first. [reference](https://github.com/CodaProtocol/signer-reference/blob/master/schnorr.ml)
///   )
