...
/// `AccountIdentifier` using the specified `SignatureType`.  `SignatureType` can be optionally
/// populated if there is a restriction on the signature scheme that can be used to sign the
/// payload.
#[derive(Clone, Debug, PartialEq, Eq, Default, Serialize, Deserialize)]
pub struct SigningPayload {
    /// [DEPRECATED by `account_identifier` in `v1.4.4`] The network-specific address of the
    /// account that should sign the payload.
...
impl SigningPayload {
    ...
    pub const fn new(hex_bytes: String) -> Self {
        Self { address: None, account_identifier: None, hex_bytes, signature_type: None }
    }
}
