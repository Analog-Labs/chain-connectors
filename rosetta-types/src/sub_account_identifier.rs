...
impl SubAccountIdentifier {
    /// An account may have state specific to a contract address (ERC-20 token) and/or a stake
    /// (delegated balance). The `sub_account_identifier` should specify which state (if applicable)
    /// an account instantiation refers to.
    #[must_use]
    pub const fn new(address: String) -> Self {
        Self { address, metadata: None }
    }
}
