...
pub struct SearchTransactionsRequest {
    ...
    /// field is not populated, the current block is considered the max_block.  If you do not
    /// specify a max_block, it is possible a newly synced block will interfere with paginated
    /// transaction queries (as the offset could become invalid with newly added rows).
...
impl SearchTransactionsRequest {
    /// `SearchTransactionsRequest` is used to search for transactions matching a set of provided
    /// conditions in canonical blocks.
    #[must_use]
