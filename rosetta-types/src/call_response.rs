...
pub struct CallResponse {
    ...
    /// Idempotent indicates that if `/call` is invoked with the same `CallRequest` again, at any
    /// point in time, it will return the same `CallResponse`.  Integrators may cache the
    /// `CallResponse` if this is set to true to avoid making unnecessary calls to the Rosetta
    /// implementation. For this reason, implementers should be very conservative about returning
