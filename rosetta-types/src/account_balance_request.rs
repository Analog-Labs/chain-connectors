...
/// `AccountBalanceRequest` : An `AccountBalanceRequest` is utilized to make a balance request on
/// the /account/balance endpoint. If the `block_identifier` is populated, a historical balance
...
impl AccountBalanceRequest {
    /// An `AccountBalanceRequest` is utilized to make a balance request on the /account/balance
    /// endpoint. If the `block_identifier` is populated, a historical balance query should be
    /// performed.
    #[must_use]
    pub const fn new(
        ...
        Self { network_identifier, account_identifier, block_identifier: None, currencies: None }
    }
}
